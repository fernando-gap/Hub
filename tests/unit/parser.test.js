...
const Message = require('../../src/server/venom/parser.js');

describe('Whatsapp Parser', function() {
  const parser = new Message();
  const simple = 'Something really interesting, ' +
  ...
  describe('Message to Whatsapp', function() {
    ...
    it('Parse Multiple messages', function() {
      const multiple = {
        body: [
          simple,
      ...
      const simpleResult = 'Something really interesting,\n' +
          'I will call you later. Don\'t be\n' +
          'sad for things that doesn\'t matter.\n' +
          'I will be really surprised if\n' +
          'someone would read it.';

      const listResult = 'You have 3 items on your list:\n' +
                   '_1._     Go for a walk.\n' +
                   '_2._     Sit on the couch.\n' +
                   '_3._     drink soda.\n';
      ...
      expect(result.text[0]).to.equal(simpleResult);
      expect(result.text[1]).to.equal(listResult);
    });
  });
});
